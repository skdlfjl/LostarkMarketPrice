...
def enter():
    ...
    # 로그인하여 로스트아크 경매장 홈페이지에 들어가집니다


def main(driver, item, eff, cha):
    # 상세 옵션 검색버튼 클릭
    ...
    driver.find_element_by_xpath('//*[@id="modal-deal-option"]/div/div/div[1]/div[1]/table/tbody/tr[3]/td[1]/div/div[2]/label[1]').click()
    ...
    driver.find_element_by_xpath('//*[@id="selEtcSub_{0}"]/div[2]/label[{1}]'.format(0, eff[0][0])).click()  # 4 (강령술) 각인 선택
    ...
    driver.find_element_by_xpath('//*[@id="selEtcSub_{0}"]/div[2]/label[{1}]'.format(1, eff[1][0])).click()  # 53 (원한) 각인 선택
    ...
    driver.find_element_by_xpath('//*[@id="selEtcSub_{0}"]/div[2]/label[{1}]'.format(2, cha[0])).click()  # 2 (치명) 특성 선택
    # 특성 선택2
    option4_x_path= '//*[@id="selEtc_3"]/div[1]'
    driver.find_element_by_xpath(option4_x_path).click()  # 기타 선택4 클릭
    ...
    driver.find_element_by_xpath('//*[@id="selEtcSub_{0}"]/div[2]/label[{1}]'.format(3, cha[1])).click()  # 3 (특화) 특성 선택
    ...
    price_buy = soup.select('div.price-buy')

    def crawling(in_list):
        for i in in_list:
